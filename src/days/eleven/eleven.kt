...
fun solveElevenDayFirstStar() {
    ...
    val stones = parseInputToStones(src1)

//    val restest1 = blink1(stones, 25).size
    // **
    val restest1 = blink1(stones.map{ it.toLong()}, 75).size
    println(restest1)
...
fun split(stone: String): List<String> =
    stone.chunked(stone.length / 2).map { it.trimStart('0').ifEmpty { "0" } }
//
...
fun split(stone: Long): List<Long> {
    ...
    return str.chunked(str.length / 2).map { it.toLong() }//.map { it.trimStart('0').ifEmpty { "0" } }
}
